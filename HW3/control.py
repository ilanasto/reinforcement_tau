...
## Value array of length NUM_STATES, initialized to small random values
value_function = np.random.uniform(0, 0.1, NUM_STATES)
## Transition probability array of shape (NUM_STATES, NUM_ACTIONS, NUM_STATES) (s, a, s'), initialized to uniform distribution
transition_probabilities = np.ones((NUM_STATES, NUM_ACTIONS, NUM_STATES)) / NUM_STATES
## Reward array of length NUM_STATES, initialized to zero
state_rewards = np.zeros(NUM_STATES)
## Initialize record the number of times `state, action, new_state` occurs
transition_counts = np.zeros((NUM_STATES, NUM_ACTIONS, NUM_STATES))
## Initialize record the rewards for every `new_state`
reward_counts = np.zeros(NUM_STATES)
## Initialize record the number of time `new_state` was reached
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    ## Bellman equation to get the best action for the current state
    action_values = np.zeros(NUM_ACTIONS)
    for action in range(NUM_ACTIONS):
        action_values[action] = np.sum(transition_probabilities[state, action, :] * (state_rewards + GAMMA * value_function))
    ...
    if new_state == NUM_STATES - 1:
        ...
        for s in range(NUM_STATES):
            for a in range(NUM_ACTIONS):
                ...
                if total_transitions > 0:
                    transition_probabilities[s, a, :] = transition_counts[s, a, :] / total_transitions
        ## update state rewards based on the average reward observed for each state
        for s in range(NUM_STATES):
            if state_counts[s] > 0:
                state_rewards[s] = reward_counts[s] / state_counts[s]
        ###### END MY CODE ######
        ...
        ###### BEGIN MY CODE ######
        # Value iteration: only count as "no learning" if the value function
        # converges in a single sweep, matching the assignment criterion.
        epsilon = float('inf')
        ...
        while epsilon > TOLERANCE:
            ...
            for s in range(NUM_STATES):
                action_values = np.zeros(NUM_ACTIONS)
                for a in range(NUM_ACTIONS):
                    action_values[a] = np.sum(transition_probabilities[s, a, :] * (state_rewards + GAMMA * value_function))
                new_value_function[s] = np.max(action_values)
                epsilon = max(epsilon, abs(new_value_function[s] - value_function[s]))
            value_function = new_value_function

            if iteration_count == 1 and epsilon < TOLERANCE:
...
plt.tight_layout()
plt.savefig('HW3/learning_curve.png', dpi=150)
plt.close()
